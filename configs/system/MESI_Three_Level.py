# This is a file I wrote casually
# Hope it works well
# If it doesn't work, I can only summon Brother Qiu Yudi
# Author:JiaoJie   2021/7/6

""" This file creates a set of Ruby caches for the MESI THREE Level protocol
This protocol models three level cache hierarchy. The L1 cache is split into
...
class MESIThreeLevelCache(RubySystem):

    def __init__(self):
        if buildEnv['PROTOCOL'] != 'MESI_Three_Level':
            fatal("This system assumes MESI_Three_Level!")

        super(MESIThreeLevelCache, self).__init__()
    ...
    def setup(self, system, cpus, mem_ctrls, dma_ports, iobus):
        ...
        #######################################################################
        #turn MESI TWO level L1 to three level L0
        #turn MESI TWO level L2 to three level L1
        #three level number L0 = number L1
        #######################################################################
        self.controllers = \
            [L0Cache(system, self, cpu) for cpu in cpus] + \
            [L1Cache(system, self, self._numL1Caches, self._numL2Caches) for cpu in cpus] + \
        ...
        #######################################################################
        #need to connect l0 and l1
        #   l1_cntrl.bufferFromL0 = l0_cntrl.bufferToL1
        #   l1_cntrl.bufferToL0 = l0_cntrl.bufferFromL1
        #maybe I can use :for (i in num_cpus)    no num_cpus
        #                      L1Cache[i].bufferFromL0 =  L0Cache[i].bufferToL1
        #######################################################################
        for i in range(len(cpus)):
            self.controllers[len(cpus)+i].bufferFromL0 = self.controllers[i].bufferToL1
            self.controllers[len(cpus)+i].bufferToL0 = self.controllers[i].bufferFromL1

        # Create one sequencer per CPU and dma controller.
        # Sequencers for other controllers can be here here.
        self.sequencers = [RubySequencer(version = i,
                                # I/D cache is combined and grab from ctrl
                                dcache = self.controllers[i].L0Dcache,
...
class L0Cache(L0Cache_Controller):
    ...
    def __init__(self, system, ruby_system, cpu):
        ...
        self.L0Icache = RubyCache(size = l0i_size,
                                assoc = l0i_assoc,
                                start_index_bit = block_size_bits ,
                                is_icache = True)
        self.L0Dcache = RubyCache(size = l0d_size,
                            assoc = l0d_assoc,
                            start_index_bit = block_size_bits,
                            is_icache = False)
...
class L1Cache(L1Cache_Controller):
    ...
    def __init__(self, system, ruby_system, num_l1Caches, num_l2Caches):

        super(L1Cache, self).__init__()

        self.version = self.versionCount()
        # This is the cache memory object that stores the cache data and tags
        self.L1cache = RubyCache(size = '512kB',
                                assoc = 16,
    ...
    def getBlockSizeBits(self, system, num_l1caches):
        l1_bits = int(math.log(num_l1caches, 2))
        bits = int(math.log(system.cache_line_size, 2)) + l1_bits
        return bits
